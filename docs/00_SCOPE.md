...
The claim ledger is authoritative. A paper statement is not silently promoted to `KERNEL_CHECKED`. Numerical laboratories are provenance and regression evidence only.

Excluded from `v1.0.0`: zero loci, special functions, analytic continuation, a logarithmic generator, spectral Weyl functions, and maximal self-adjoint extensions.
